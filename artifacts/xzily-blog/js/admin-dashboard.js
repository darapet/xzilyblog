...
function render() {
  const posts = store.getPosts({ status: 'all' });
  const published = posts.filter((p) => p.status === 'published');
  const totalViews = posts.reduce((s, p) => s + p.views, 0);
  const totalLikes = posts.reduce((s, p) => s + p.likes, 0);
  const subscribers = store.getSubscribers().length;
  ...
  const comments = store.allComments().sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)).slice(0, 6);
  document.getElementById('activityFeed').innerHTML = comments.map((c) => `
    <div style="display:flex;gap:12px;align-items:flex-start;">
      ...
      <div>
        <div style="font-size:13.5px;"><strong>${(store.userById(c.authorId)?.name) || c.authorName || 'Guest'}</strong> commented</div>
